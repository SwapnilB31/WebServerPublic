...
#The URL to the Windows Server 
host_url = "http://ec2-13-127-179-41.ap-south-1.compute.amazonaws.com/"
...
def fetch_all_addUser():
    ...
    for user_dict in data:
        ...
        if len(res) == 0:
            ...
            val = (enabled,given_name,name,obj_class,obj_guid,sam_acc_name,sid,surname,user_principal_name)
            ...
            for part in parts:
                ...
                if field == "CN":
                    ...
                    sql = "INSERT IGNORE INTO cnuserrel(`cnid`,`SID`) VALUES ((SELECT `id` FROM cn WHERE `name` = '"+param+"'),'%s')" % (sid)
                    cursor.execute(sql)
                    db.commit()
                    ...
                    sql = "INSERT IGNORE INTO dcuserrel(`dcid`,`SID`) VALUES ((SELECT `id` FROM dc WHERE `name` = '"+param+"'),'%s')" % (sid)
                    cursor.execute(sql)
                    db.commit()
        ...
        if len(res) == 0:
            ...
            for part in parts:
                ...
                if field == "CN":
                    ...
                    sql = "INSERT IGNORE INTO cnadminrel(`cnid`,`SID`) VALUES ((SELECT `id` FROM cn WHERE `name` = '"+param+"'),'%s')" % (sid)
                    cursor.execute(sql)
                    db.commit()
                    ...
                    sql = "INSERT IGNORE INTO dcadminrel(`dcid`,`SID`) VALUES ((SELECT `id` FROM dc WHERE `name` = '"+param+"'),'%s')" % (sid)
                    cursor.execute(sql)
                    db.commit()
...
def fetch_user_password_expiry():
    ...
    for key in data:
        if data[key] is not None:
            sql = "UPDATE `aduser` SET `PasswordExpiry` = '%s' WHERE `SID` = '%s'" %(data[key],key)
            cursor.execute(sql)
            db.commit()
        else:
            sql = "UPDATE `aduser` SET `PasswordExpiry` = NULL WHERE `SID` = '%s'" %(key)
            cursor.execute(sql)
            db.commit()
...
def fetch_data():
    return fetch_all_addUser() and fetch_user_password_expiry() and fetch_admin_principal_memberships()
...
def randomizeLastLogin():
    ...
    for i in range(len(res)):
        ...
        sql = "UPDATE `aduser` SET `lastLogin` = '%s' WHERE `user_id` = '%s'" %(last_login_str,user_id)
        cursor.execute(sql)
        db.commit()
...
def randomizePasswordExpiry():
    ...
    for i in range(len(res)):
        ...
        last_login = datetime.now() + timedelta(days = day_diff)
        last_login_str = last_login.strftime("%y-%m-%d %H:%M:%S")
        sql = "UPDATE `aduser` SET `PasswordExpiry` = '%s' WHERE `user_id` = '%s'" %(last_login_str,user_id)
        cursor.execute(sql)
        db.commit()
